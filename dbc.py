...
print(cursor)
cursor.execute('''
                INSERT INTO FileSearchResults2_table1 (File_Location, SearchCount, NameOfFile)
                VALUES
                ('C:\mainfolder\emptyfile.txt',1,'emptyfile') ''')
cnxn.commit()

values=cursor.execute('select * from FileSearchResults2_table1')
for i in values:
    print(i)
